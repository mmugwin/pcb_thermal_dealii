...
{
      double return_value = 5;        // This is the source
      return return_value;
}
...
void pcbSteadyState<dim>::make_grid_from_vtk()
{
  ...
  std::ifstream f("testPCBMesh.vtk");      //let's read in the vtk input file
  pcb_input_mesh.read_vtk(f);
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
      {
        ...
        // Here is where we apply the Neumann Boundary Conditions    
        for (const auto &face : cell->face_iterators())
...
{
  {
    pcbSteadyState<3> laplace_problem_3d;
    laplace_problem_3d.run();
    pcbSteadyState<3> test_mesh;
    test_mesh.make_grid_from_vtk();
  }
  return 0;
}
